    ...
    @if (cv(); as data) {
      @if (data.education.length || data.languages.length) {
        <section class="max-w-[980px] mx-auto px-6 pb-24 grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-5">
          @if (education(); as edu) {
            <div class="p-6.5 rounded-[18px] glass-card" scrollReveal="spring-up">
          ...
          @if (data.languages.length) {
            <div class="p-6.5 rounded-[18px] bg-[#15151A] text-[#FAFAF7] neo-border neo-shadow-lime" scrollReveal="spring-up" [revealDelay]="100">
